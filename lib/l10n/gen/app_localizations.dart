...
abstract class AppLocalizations {
  ...
  /// The same, for a collection whose size the server did not send. ChildCount is absent unless Fields=ChildCount is asked for, and a tile in that state renders no count badge either — so the spoken label degrades the same way rather than saying 'nought titles'.
  ///
  /// In en, this message translates to:
  /// **'Collection'**
  String get librarySemanticCollectionAlone;

  /// Header on the screen showing what is inside one collection. A fact about the library, not about the selected child, so unlike the grid's result line it does not change when the selection does.
  ///
  /// In en, this message translates to:
  /// **'{count} titles in this collection'**
  String collectionTitleCount(int count);

  /// Button on the collection screen. This is the action that tapping a collection tile used to perform on its own; it opens the same write preview, which still shows the diff and writes nothing until Apply. Disabled until a child is picked, because the preview is for a child.
  ///
  /// In en, this message translates to:
  /// **'Give the whole set'**
  String get collectionGiveWholeSet;

  /// Shown when a collection has no members. Rare but real: a set can be emptied on the server while Garfin is looking at it.
  ///
  /// In en, this message translates to:
  ...
  /// Chip after the avatars on a tile, for the children who did not fit in the row. Shown as +2.
  ///
  /// In en, this message translates to:
  /// **'+{count}'**
  ...
  /// The honesty note. Measured: Jellyfin records nothing for a metadata edit, so there is no history to read back — a log that looked complete would be worse than no log.
  ///
  /// In en, this message translates to:
  ...
  /// Wraps the windows with whose clock they are on. Measured: the API exposes the server's UTC instant and nothing about its offset, so Garfin cannot convert these into the phone's time and must not imply it has.
  ///
  /// In en, this message translates to:
  ...
  /// Measured: outside the window the approval and the exchange both succeed, and the session then answers 403 on every request until the hours begin. Garfin cannot tell whether now is inside the window — the server's offset is not exposed — so this says what happens rather than predicting which case applies.
  ///
  /// In en, this message translates to:
  ...
  /// 'Asked', for the same reason as sent: a 204 is the server accepting the command, not the client obeying it. Since #70 this is what the toast says while the read-back is in flight, replaced by sessionsStopStopped or sessionsStopIgnored once /Sessions has been asked.
  ///
  /// In en, this message translates to:
  /// **'Asked {device} to stop.'**
  String sessionsStopSent(Object device);

  /// Read back from /Sessions a few seconds later (#70): the session is there and nothing is playing on it, or the session is gone. The one thing the old copy could never say.
  ///
  /// In en, this message translates to:
  /// **'{device} stopped playing.'**
  String sessionsStopStopped(String device);

  /// Read back from /Sessions a few seconds later (#70): the command was accepted and the client did not act on it. Says the fact rather than blaming the client, because a session that moved to a different title reaches this branch too. No advice attached — there is nothing over this API that would make it obey.
  ///
  /// In en, this message translates to:
  /// **'Asked {device} to stop, but it\'s still playing.'**
  ...
  /// Read back from /Sessions a few seconds later (#70): a client holding saved credentials re-authenticates, and because sessions are keyed by device the new one wears the same name. Without this the list looks unchanged and the parent concludes the button did nothing.
  ///
  /// In en, this message translates to:
  ...
  /// The two-pane library's panel before anything is picked (#95). The panel is always on screen at tablet widths so that picking a title does not re-flow the grid, which means it needs something to say when it is empty. Describes what the space is for rather than instructing — and says 'would change', because ground rule 1 means nothing is written until Apply.
  ///
  /// In en, this message translates to:
  ...
  ///
  /// In en, this message translates to:
  /// **'Nothing here is {name}\'s yet'**
  String collectionGivenNone(String name);

  /// The state #107 exists for: part of a set handed over, which nothing on any screen used to say. A count of *labels*, never of what the child can see — a rating cap can hide a labelled title, so this sentence and 'sees N of M' are different facts and never share a line.
  ///
  /// In en, this message translates to:
  /// **'{given} of {total} given to {name}'**
  String collectionGivenSome(int given, int total, String name);

  /// The whole set. Its own string rather than a plural of the partial one, because 'All 8' is the sentence a parent is looking for and '8 of 8' reads like a coincidence.
  ///
  /// In en, this message translates to:
  /// **'All {total} given to {name}'**
  String collectionGivenAll(int total, String name);

  /// The same three states for a block-list child, where the identical label means the opposite (ground rule 3). One count with one verb would be exactly backwards for every child in that mode, and nothing on screen would look wrong.
  ///
  /// In en, this message translates to:
  /// **'Nothing here is kept from {name}'**
  String collectionKeptNone(String name);

  /// Block mode: the label withholds rather than gives. Same count, opposite verb.
  ///
  /// In en, this message translates to:
  /// **'{kept} of {total} kept from {name}'**
  String collectionKeptSome(int kept, int total, String name);

  /// Block mode, whole set.
  ///
  /// In en, this message translates to:
  /// **'All {total} kept from {name}'**
