...
> **Scope, stated rather than left implied.** That run measured a build
...
**The exception, added deliberately: About → Check for updates.** One
...
**That includes the platform's own egress, which is off (2026-08-05).**
...
which stopped being true when the client landed.
...
**Re-grepped, because that change falsified two sentences here.** There
...
- `app_info.dart`'s `sourceUrl`, which since it is *opened* rather than only
...
already bitten this project once, see § Release signing.
...
**Approving a child's sign-in never brings a secret near Garfin.** The
...
**Verified against the implementation (2026-08-04).** Two ways, because a
static reading and a behavioural one fail differently.

*Statically*, every write in `lib/` was enumerated — **twenty-two
`shared_preferences` call sites across six files** (2026-08-06, since
updated), which is the complete list:
...
| `unlock_settings_store.dart` | 3 | whether the unlock gate is required (bool), the idle timeout (int seconds), whether the one-time unlock question has been answered (bool) |
| `device_identity.dart` | 1 | `device_id` — see below |

> **This count was wrong for a time.** It read "eleven across four
...
device-to-device transfer are off, so it does not leave the device.
...
transfer were switched off, not before. Had the order been reversed, the
...
- The token remains in `flutter_secure_storage` either way; the gate covers the *session*, not
  the storage.
- **A signed-out Garfin shows the remembered server URL without asking.** The gate used to
...
The default branch requires a pull request with passing checks, and tag creation, deletion and
force-update are restricted.

Read that honestly, though: **a repository admin can push past it.** The protection is against
mistakes and against non-admin contributors, not against a compromised admin account. Anyone
modelling the threat here should assume admin access defeats it.
